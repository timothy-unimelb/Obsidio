...
// are REUSED unchanged: every request is dispatched through route(), and the
// writeJSON helpers short-circuit into rawResponse, so the gate's response and
// error accounting is identical on both servers.
//
// Ported from Advait's branch (app/rawserver.go) with the dispatch adapted to
// this package's single route() entry point. Kill switch: RISK_HTTP=std boots
// the stock net/http server instead (see main()).
//
