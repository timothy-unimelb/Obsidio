...
test("exports the solution page", async () => {
  ...
  assert.match(html, /workers = 2 · lanes = 4 · yield = 256 · shed = 88bp/);
...
test("exports the performance record", async () => {
  ...
  assert.match(html, /2,545,521/);
  assert.match(html, /3,598,675/);
  assert.match(html, /4,854,704/);
  assert.match(html, /\+53\.8%/);
  assert.match(html, /\+17\.2%/);
  assert.match(html, /Budgeted shedding governor/);
  assert.match(html, /HEAD TO HEAD/);
  assert.match(html, /4,321,831/);
  assert.match(html, /Permanent workers \+ queue/);
  assert.match(html, /Packed lowercase hex/);
  assert.match(html, /Two-lane SHA-NI kernel/);
  ...
  assert.match(html, /ZERO-ERROR → GOVERNOR/);
