...
test("exports the solution page", async () => {
  ...
  assert.match(html, /workers = 2 · lanes = 4 · yield = 256/);
...
test("exports the performance record", async () => {
  ...
  assert.match(html, /INSIDE A PEAK \/PRICE REQUEST/);
  assert.match(html, /17\.6 ms/);
  assert.match(html, /KERNEL → KERNEL \+ YIELD/);
